...
**Corrección 2026-09-20:** el store de producción (`contaflow-blob`) es **privado** y rechaza `access: 'public'` ("Cannot use public access on a private store", comprobado contra el store). Esta feature no ha podido funcionar hasta migrarla a acceso privado: ver ADR-047. Lo que sigue describe el código actual, no lo desplegado.

**Riesgo residual (sin cambios en el código).** El blob sigue siendo `access: 'public'`: la URL es un bearer sin revocación, el soft-delete (D-6) deja el blob vivo y el CDN lo cachea ~1 mes. Solución de fondo pendiente: store Blob privado + ruta de descarga autenticada con `requireCompanyAction` que haga stream con `get()`; hay que migrar también `analyzeReceiptAction` (`payment.actions.ts`, hace `fetch(blobUrl)`). No bloquea un Alpha con un solo tenant; sí es requisito antes del segundo tenant o GA.
