...
### ⚠️ PENDIENTES PRÓXIMA SESIÓN (rate-limit lecturas + badge anomalías)
- **#2 RIESGO PRE-LANZAMIENTO — rate-limit en lecturas:** `limiters.fiscal` es `slidingWindow(10, "1 m")` (¡solo 10/min!, keyed por userId) y lo comparten ~53 acciones, incluidas **lecturas de render** (KPIs `kpi-dashboard.actions`, `pending-tasks.actions`, reportes `report.actions` journal/ledger/trial-balance). Un usuario activo en prod (si Upstash está configurado) puede agotarlo navegando rápido → "Demasiadas solicitudes." Fix correcto: crear `limiters.read` generoso (~120/min) en `src/lib/ratelimit.ts` y migrar a él las lecturas de render (NO tocar las mutaciones fiscales, que deben seguir a 10/min — Z-1). Toca varias Server Actions → pasar security-agent. Ya se quitó el rate-limit de `getDashboardMetricsAction` (causaba redirect a /dashboard).
