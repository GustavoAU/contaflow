...
- **#3 Badge anomalías vía Route Handler 2026-06-14** ✅ merged — Restaurado el badge de `FloatingAIAssistant` sin reintroducir el bug de hooks: nueva ruta `GET /api/company/[companyId]/anomaly-summary` (`route.ts`) que reusa `getAnomalySummaryAction` (auth + IDOR + rol ACCOUNTING + degradación graceful) y añade rate limit `limiters.read` (120/min, fiscalKey). FloatingAIAssistant la consume con `fetch()` lazy en `useEffect` (AbortController, `.catch` silencioso) — NO Server Action, así no pasa por la cola de Next (useActionQueue/useOptimistic). `initialAnomaly` queda como estado inicial opcional. tsc 0 | 2782 tests GREEN. Auditoría de seguridad de la nueva ruta: ver abajo.
- **Rate-limit lecturas — limiters.read 2026-06-14** ✅ merged (`feat/limiters-read-rate-limit` + tanda F `0391dc3`) — Nuevo `limiters.read` = `slidingWindow(120,"1 m")` (prefix `rl:read`, fail-open) en `src/lib/ratelimit.ts`, separado de `limiters.fiscal` (10/min, fail-closed, mutaciones — Z-1). Migradas **16 lecturas/exports de render** que compartían el limitador fiscal y bloqueaban a usuarios/auditores ("Demasiadas solicitudes"/redirect): getDashboardMetricsAction, getKpiDashboardAction, getPendingTasksAction, guardAccounting (reportes Diario/Mayor/Balance), getAccountsAction, getNextAccountCodeAction, getTransactionsByCompanyAction, getInvoiceBookAction (Libro Ventas), listExpensesAction, listExpenseCategoriesAction, getFixedAssetGLReconciliationAction, getNotificationsAction, listPaymentBatchesAction, listUnpaidPurchaseInvoicesAction, listPaymentsAction, getPaymentsByInvoiceAction + exports PDF/XML on-demand (factura voucher/XML, estados financieros, aging). **Beneficio SENIAT:** auditor revisando muchos libros ya no se topa con el límite de 10/min. Mutaciones intactas en `fiscal`. Auditoría de seguridad (2 pasadas, playbook security-agent): **GO**, 0 CRITICAL/HIGH; verificado ninguna mutación movida. Nota concurrencia: tanda F empujada a main por fast-forward (`fe125f1..0391dc3`) sin checkout, por haber otro agente en el working tree compartido (`feat/auth-payment-harmony`, UI/Clerk). tsc 0 | **2782 tests GREEN**. Pendiente futuro: aún hay lecturas costosas en exports masivos (`limiters.export`) y `account.findMany` sin take en dashboard (optimizar a aggregate SQL).
- **Fix dashboard empresa (hooks Next + rate-limit lectura) 2026-06-13** ✅ merged (`fix/company-dashboard-hooks-ratelimit`, commits `df26d78`+`3d73ec7`) — Dos bugs que impedían abrir `/company/[companyId]`: (1) **"Rendered more hooks"** = bug interno de Next (`Router`/`useActionQueue`/`useOptimistic` en `app-router.tsx`) disparado al despachar Server Actions en `useEffect` durante el montaje de `BcvRateWidget` (`getLatestRatesWithDeltaAction`) y `FloatingAIAssistant` (`getAnomalySummaryAction`). Fix: tasas BCV obtenidas en el server (layout) → props iniciales; badge anomalías diferido (`initialAnomaly=null`, ver pendiente #3). (2) **Rebote a /dashboard** = `getDashboardMetricsAction` (lectura de render) usaba `limiters.fiscal` (10/min) → al agotarse devolvía "Demasiadas solicitudes" y la página redirigía. Fix: quitado rate-limit de esa lectura (sigue con auth + IDOR). Diagnóstico clave: el stack apuntó a `app-router.tsx` (Next), no a código propio — ESLint `rules-of-hooks` limpio en todo el repo. tsc 0 | **2782 tests GREEN**.
- **UI Gaps responsive/EmptyState/error-boundaries 2026-06-13** ✅ merged (`feat/ui-gaps-responsive-empty-docs`, commits `65cffef`+`29a5902`) — Auditoría ui-agent/ui-patterns vs app. Gap #1 (tablas responsive, enfoque Mixto): utilidad CSS reutilizable `stack-card-table` en `globals.css` (apila a tarjetas <lg vía `data-label`, sin duplicar JSX) aplicada a CustomerList/VendorList/OrderList/QuotationList; 11 wrappers `overflow-hidden`→`overflow-x-auto` en tablas densas `min-w-full` que se recortaban en móvil (payroll, inventario, reportes, fiscal-calendar). Gap #2: `EmptyState` en 6 listas primarias (RetentionList, PaymentRecordList, InventoryItemList, TransactionList, EmployeeList, IncomeDistributionList). Gap #3: `ui-patterns.md` v1.1 + `ui-agent.md` sincronizados con nombres reales (PageTransitionBar, usePageTransition en PageTransitionProvider, *PageSkeleton; eliminados PageTransitionLoader/DataTableSkeleton/LoadingSpinner inexistentes) + §6 estrategia de tablas; contraste `EmptyState` `text-zinc-400`→`zinc-600` (WCAG AA). Gap #4: `error.tsx` con auto-retry de cold-start en invoices/payroll/fiscal-close/retentions. Decisión tablas densas: scroll horizontal (no cards), consistente con 2026-06-01. `.claude/settings.local.json` añadido a `.gitignore`. tsc 0 errores | **2783 tests GREEN**.
- **Sprint Softnetcorp F4/F5/F7/F8/F9/F10 + préstamos ext 2026-06-05** ✅ merged (`feat/nom-prestamos-reportes-ext`) — Préstamos: `LoanStatus` PENDING/REJECTED + flujo aprobación (ACCOUNTING crea PENDING → ADMIN aprueba/rechaza); interés opcional con método francés (tasa anual, cliente y servidor); currency MIXED (parte VES + parte USD); `PayrollRunService.approve` actualiza `remainingBalanceUsd` en MIXED. Reportes: Excel export IVSS/BANAVIH/INCES (ExcelJS); TXT BANAVIH FAOV-Web (pipe-delimited); CSV MINTRA declaración trimestral (semicolón, activos+egresados); PDF Forma 14-100 Constancia de Trabajo individual. UI: doble moneda en `PayrollRunDetail` (badge 1 USD=Bs. + equiv. USD en 4 tarjetas). 2359 tests GREEN.
- **Sprint Softnetcorp F4/F5/F7/F8/F9/F10 base 2026-06-05** ✅ merged (`feat/nom-vacaciones-semanal-import`) — SEMANAL en PayrollFrequency; VacationRequest model PENDING/APPROVED/REJECTED/CANCELLED + balance LOTTT Art.190 acumulado + flujo aprobación manager; EmployeeHistoricalImportDialog (días vacaciones + prestaciones); payslip email fire-and-forget post-aprobación; ManagerApprovalInbox vacaciones pendientes; /payroll/vacation-requests page. 2359 tests GREEN.
