...
- **#3 — restaurar badge de anomalías de FloatingAIAssistant:** quedó diferido (`initialAnomaly=null`) tras el fix de hooks. Restaurar vía **Route Handler** (`GET /api/.../anomaly-summary`) consumido con `fetch()` lazy post-montaje (no pasa por la cola de acciones de Next → no reintroduce el bug; no bloquea el layout). Nueva ruta API → pasar security-agent.
