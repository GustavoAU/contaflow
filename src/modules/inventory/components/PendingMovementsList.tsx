...
import { LotSerialModal, type LotSerialInput } from "./LotSerialModal";
...
export function PendingMovementsList({ movements, companyId, canPost }: Props) {
  ...
  const [modalMovement, setModalMovement] = useState<PendingMovement | null>(null);

  function executePost(movementId: string, extraData?: LotSerialInput) {
    setPostingId(movementId);
    startPost(async () => {
      const r = await postMovementAction({ movementId, companyId, ...extraData });
      setPostingId(null);
      if (r.success) {
  ...
  function handlePost(mov: PendingMovement) {
    if (mov.item.trackingType === "NONE") {
      if (
        !confirm(
          `¿Contabilizar el movimiento de "${mov.item.name}"?\nSe generará un asiento contable automático y el stock se actualizará.`
        )
      )
        return;
      executePost(mov.id);
    } else {
      setModalMovement(mov);
    }
  }

  function handleModalConfirm(data: LotSerialInput) {
    if (!modalMovement) return;
    const movId = modalMovement.id;
    setModalMovement(null);
    executePost(movId, data);
  }
    ...
    <>
      {modalMovement && (
        <LotSerialModal
          ...
          isSubmitting={postingId === modalMovement.id && isPendingPost}
        />
      )}
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {movements.map((mov) => {
                  ...
                  const needsTracking = mov.item.trackingType !== "NONE";
