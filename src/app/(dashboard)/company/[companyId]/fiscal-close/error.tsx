...
export default function FiscalCloseError({ error, reset }: Props) {
  const params = useParams<{ companyId: string }>();
  const [countdown, setCountdown] = useState(4);

  useEffect(() => {
    console.error("[fiscal-close/error]", error.message);
  }, [error]);
    ...
    <div className="flex flex-col items-center justify-center py-20 text-center">
      <div className="mx-auto max-w-md space-y-4">
        ...
        <h2 className="text-lg font-semibold text-zinc-800">
          {isColdStart ? "La base de datos está iniciando" : "Error en el cierre fiscal"}
        </h2>

        <p className="text-sm text-zinc-500">
          {isColdStart
            ? `Reconectando automáticamente en ${countdown}s…`
            : "Ocurrió un error al cargar el cierre fiscal. Puedes reintentar o volver al módulo."}
        </p>
        ...
        <div className="flex justify-center gap-3 pt-2">
          ...
          {params?.companyId && (
            <Link
              href={`/company/${params.companyId}/fiscal-close`}
